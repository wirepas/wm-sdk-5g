...
#elif RX_BUFFER_SIZE == 511
static uint16_t m_last_rx_index = 0;
#else
#error Unsupported RX_BUFFER_SIZE
#endif
...
void Usart_receiverOn(void)
{
    ...
    // Prepare RX buffer
    NRF_UARTE0_RX_PTR = (uint32_t) m_rx_circular_buffer;
    NRF_UARTE0_RX_MAXCNT = RX_BUFFER_SIZE;
    m_last_rx_index = 0;
