 ...
 *          from desired GPIO pin range
 * \param   pin_min
 *          start of the pin range to be checked
 * \param   pin_max
 *          end of the pin range to be checked
 * \return  True if any of the  GPIO "SENSE" is enabled ; False otherwise
 */
static bool any_sense_enabled(gpio_pin_t pin_min,
                              gpio_pin_t pin_max);

/** \brief function called when a GPIO interrupt is raised. Used to generate the appropriate GPIO event */
static void gpio_event_handler_gpiote_isr(void);
#ifdef NRF_GPIOTE_LP
static void gpio_event_handler_gpiote_lp_isr(void);
#endif
...
static void input_set_cfg_irq(gpio_id_t id, const gpio_in_cfg_t *in_cfg)
{
...
#ifdef NRF_GPIOTE_LP
        /* if not a single GPIO uses its sense feature from GPIO port 0, ... */
        if(!any_sense_enabled(GPIO_PIN_ID_LP_MIN, GPIO_PIN_ID_LP_MAX))
        {
            /* ...then disable the port interrupt (since it is no longer used by GPIO port 0) */
            nrf_gpiote_int_disable(NRF_GPIOTE_LP, GPIOTE_INTENCLR_PORT_Msk);
        }
        /* if not a single GPIO uses its sense feature from GPIO port 1, ... */
        if(!any_sense_enabled(GPIO_PIN_ID_MIN, GPIO_PIN_ID_MAX))
        {
            /* ...then disable the port interrupt (since it is no longer used by GPIO port 1) */
    ...
    else
    {
        ...
        nrf_gpio_pin_latch_clear(pin);
#ifdef NRF_GPIOTE_LP
/* Allow pin ID range check to have "always true" conditions,
   like 0 is always greater or equal to 0. */
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
        if ((pin>=GPIO_PIN_ID_LP_MIN) && (pin<=GPIO_PIN_ID_LP_MAX))
        {
            nrf_gpiote_int_enable(NRF_GPIOTE_LP, GPIOTE_INTENCLR_PORT_Msk);
        }
        else
        {
...
static void input_init_irq(void)
{
    ...
    lib_system->enableAppIrq(true,
                             ...
                             gpio_event_handler_gpiote_isr);
    lib_system->clearPendingFastAppIrq(GPIOTE_IRQn);
#ifdef NRF_GPIOTE_LP
    // This chip has also GPIOTE device in low power domain
    nrf_gpiote_event_clear(NRF_GPIOTE_LP, NRF_GPIOTE_EVENT_PORT);
    nrf_gpiote_int_disable(NRF_GPIOTE_LP, GPIOTE_INTENCLR_PORT_Msk);
    lib_system->enableAppIrq(true,
                             GPIOTE_LP_IRQn,
                             APP_LIB_SYSTEM_IRQ_PRIO_LO,
                             gpio_event_handler_gpiote_lp_isr);
    lib_system->clearPendingFastAppIrq(GPIOTE_LP_IRQn);
#endif
}
                              ...
                              gpio_pin_t pin_max)
{
    ...
    for (id = 0; id < BOARD_GPIO_NUMBER; id++)
    {
        ...
        {
            // Do not care about GPIO pins that are not from wanted range.
            continue;
        }
...
static void gpio_event_handler(NRF_GPIOTE_Type * gpiote)
{
    gpio_id_t id;
    gpio_pin_t pin;
    ...
    if (!nrf_gpiote_event_check(gpiote, NRF_GPIOTE_EVENT_PORT))
    {
        return;
    }

    /* clear the port interrupt */
    nrf_gpiote_event_clear(gpiote, NRF_GPIOTE_EVENT_PORT);
...
static void gpio_event_handler_gpiote_isr(void)
{
    gpio_event_handler(NRF_GPIOTE);
}

#ifdef NRF_GPIOTE_LP
static void gpio_event_handler_gpiote_lp_isr(void)
{
    gpio_event_handler(NRF_GPIOTE_LP);
}
#endif // NRF_GPIOTE_LP
