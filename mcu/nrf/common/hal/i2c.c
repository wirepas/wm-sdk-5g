...
static void configure_gpios(bool pullup)
{
    nrf_gpio_pin_pull_t pull
        = (pullup ? NRF_GPIO_PIN_PULLUP : NRF_GPIO_PIN_NOPULL);
    ...
    // Configure SCL
    nrf_gpio_cfg(BOARD_I2C_SCL_PIN,
                 NRF_GPIO_PIN_DIR_INPUT,
                 NRF_GPIO_PIN_INPUT_CONNECT,
                 pull,
                 NRF_GPIO_PIN_S0D1,
...
{
    I2C_DEV->INTENCLR = I2C_DEV_INTENCLR_ALL;
}
...
i2c_res_e I2C_transfer(i2c_xfer_t * xfer_p, i2c_on_transfer_done_cb_f cb)
{
    ...
    {
        I2C_DEV_TX_PTR    = (uint32_t) m_current_xfer.client_xfer->write_ptr;
        I2C_DEV_TX_MAXCNT = m_current_xfer.client_xfer->write_size;
    }

    if (m_current_xfer.client_xfer->read_ptr != NULL)
    {
        I2C_DEV_RX_PTR    = (uint32_t) m_current_xfer.client_xfer->read_ptr;
        I2C_DEV_RX_MAXCNT = m_current_xfer.client_xfer->read_size;
    }
    ...
    if (m_current_xfer.client_xfer->write_ptr != NULL)
    {
        I2C_DEV_TASKS_STARTTX = 0x1UL;
    }
    else
    {
        I2C_DEV_TASKS_STARTRX = 0x1UL;
    }
