    ...
    if (m_events.start)
    {
        ...
        else
        {
            ...
            if (m_conf.method == PROV_METHOD_SECURED ||
                m_conf.method == PROV_METHOD_EXTENDED_UID)
            {
...
provisioning_ret_e Provisioning_init(provisioning_conf_t * conf)
{
    ...
    if (m_conf.method == PROV_METHOD_KEY_MGMT)
    {
        /* Scalable key management method */

        if (m_conf.p_key_mgmt_config == NULL)
        {
            return PROV_RET_INVALID_PARAM;
        }
        ...
        app_res_e res
            = lib_settings->keyManagementConfiguration(m_conf.p_key_mgmt_config);

        if (res != APP_RES_OK)
        {
            LOG(LVL_ERROR,
                "Failed to enable scalable key management, res: %d",
                res);

            return PROV_RET_INVALID_PARAM;
        }
        LOG(LVL_INFO, "Key management enabled");
        ...
        ret = Provisioning_Failsafe_init(&failsafe_conf);
        if (ret != PROV_RET_OK)
        {
            return ret;
        }

        LOG(LVL_DEBUG, "Failsafe timeout: %dsec", failsafe_conf.timeout_sec);
