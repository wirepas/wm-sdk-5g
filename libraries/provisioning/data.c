...
{
    if (m_method == PROV_METHOD_KEY_MGMT)
    {
...
static void apply_network_parameters(void)
{
    ...
    if (m_method == PROV_METHOD_KEY_MGMT)
    {
        LOG(LVL_DEBUG, " - Network key sequence number: %d",
                      m_provisioning_key_mgmt_data.net_key_seq);

        LOG(LVL_DEBUG, " - Management encryption key : * * * * * * * * * * * *"
                       " %02X %02X %02X %02X",
                       m_provisioning_key_mgmt_data.mgmt_enc_key[12],
                       m_provisioning_key_mgmt_data.mgmt_enc_key[13],
        ...
        LOG(LVL_DEBUG,
            ...
            " %02X %02X %02X %02X",
            m_provisioning_key_mgmt_data.mgmt_auth_key[12],
            m_provisioning_key_mgmt_data.mgmt_auth_key[13],
            m_provisioning_key_mgmt_data.mgmt_auth_key[14],
            m_provisioning_key_mgmt_data.mgmt_auth_key[15]);

        app_lib_settings_key_management_configuration_t key_management_config = {
            ...
            .management_key_pair.encryption_key_p = &m_provisioning_key_mgmt_data.mgmt_enc_key,
            .management_key_pair.authentication_key_p = &m_provisioning_key_mgmt_data.mgmt_auth_key,
            .network_key_pair_seq = m_provisioning_key_mgmt_data.net_key_seq,
            // future support
            //.management_key_pair_seq = m_provisioning_key_mgmt_data.mgmt_net_key_seq,
            .management_key_pair_seq = 255,
            .flags = 
            {
                .key_management = true,
                .revoke_keys = false,
        ...
        if (lib_settings->keyManagementConfiguration(&key_management_config) != APP_RES_OK)
        {
