...
/** Joining proxy library is started. */
...
void send_nack(prov_nack_type_e type, const app_lib_data_received_t * data)
{
    ...
    app_lib_data_to_send_t data_to_send =
    {
        ...
        .src_endpoint = PROV_DOWNLINK_SRC_EP,
        .dest_endpoint = PROV_DOWNLINK_DST_EP,
    };
                                        ...
                                        const app_lib_data_received_t * data)
{
    ...
    /* Check method. */
    if (PROV_METHOD_IS_UNSECURED(pdu->method))
    {
    ...
    else if (PROV_METHOD_IS_SECURED(pdu->method))
    {
    ...
    if (PROV_METHOD_IS_SECURED(pdu->method))
    {
...
provisioning_ret_e Provisioning_Proxy_init(provisioning_proxy_conf_t * conf)
{
    ...
    if (is_local_provisioning)
    {
        ...
        m_ptk_received_item.filter.src_endpoint = PROV_UPLINK_SRC_EP;
        m_ptk_received_item.filter.dest_endpoint = PROV_UPLINK_DST_EP;
...
#ifndef MAC_PROFILE_DECTNR
    param.payload = m_conf.payload;
    param.payload_num_bytes = m_conf.num_bytes;
#endif
