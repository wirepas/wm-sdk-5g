 ...
 *          to specifically address local provisioning need
 */
...
/** Pre shared key used to encrypt/decrypt provisioning messages */
...
/**
 * \brief   Select the joining beacon based on RSSI and validates PSK ID
 * \param   beacons
 *          A pointer to the first received beacon or NULL.
 * \return  Strongest beacon or NULL
 */
static const app_lib_joining_received_beacon_t * joining_beacon_selection(
    const app_lib_joining_received_beacon_t * beacons)
{
    ...
    while (beacons != NULL)
    {
            ...
            && (m_psk.id == *((uint32_t *) beacons->payload)))
        {
            ...
            LOG(LVL_DEBUG, "Discard beacon with incorrect PSK ID: 0x%08x", *((uint32_t *) beacons->payload));
        }
        beacons = beacons->next;
    }

    if (strongest_beacon != NULL) {
        LOG(LVL_DEBUG, "Found join beacon with expected PSK ID");
    }
...
static void enable_proxy(void)
{
    ...
    {
        // A node that is not fully provisioned
                                                 ...
                                                 local_provisioning_proxy_enabled_cb on_proxy_enabled_cb)
{
    ...
    /* Use them for now until override by app config for proxy*/
    memcpy(&m_psk, &m_psk_init, sizeof(m_psk));
...
{
    // In local provisioning, node UID can be anything as it just have to be unique locally
    ...
    provisioning_conf_t conf =
    {
        ...
        .user_data_cb = NULL,
        .beacon_joining_cb = joining_beacon_selection
    };

    if (cb_beacons != NULL)
    {
        // Override default joining beacon callback
        conf.beacon_joining_cb = cb_beacons;
    }

    if (Local_provisioning_is_provisioned())
    {
        // Node is already provisioned, and must be un-provisioned first
    ...
    // trigger a normal network scan by design making our first joining
    // beacon failing always
