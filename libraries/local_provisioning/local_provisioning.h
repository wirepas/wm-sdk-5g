 ...
 * \note    As stack must be started to try to join a network, it must be stoped at the end
 *          in case provisioning fails, otherwise network will scan on a network that doesn't
 *          exist. But it will generate a reboot. If not desired and main powered, stack can
 *          stay started
 */
 ...
 * \note    This call will generate a reboot of node and will never return
 */
