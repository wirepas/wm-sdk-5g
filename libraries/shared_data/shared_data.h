 ...
 * modules in the application. It provides helper functions to facilitate operations
 * on data packets. It calls underlying functions and callbacks from Wirepas 
 * Stack (api/wms_data.h).
 *
 * This library replaces the following functions and callbacks of the data
 * library from the Wirepas Stack:
 *  - [lib_data->setDataReceivedCb()](@ref app_lib_data_set_data_received_cb_f),
 *  - [lib_data->setDataSentCb()](@ref app_lib_data_set_data_sent_cb_f)
 *  - [lib_data->sendData()](@ref app_lib_data_send_data_f)
 * 
 * If using this helper, above callbacks MUST NOT be used in your own code. Using
 * above callbacks prevents the Helper Library accessing the data packets. 
 * Hence, you should keep using functions from this helper. 
 *
