...
static const uint8_t m_attr_size_lut[27] = {
    [CSAP_ATTR_NODE_ID - 1] = CSAP_ATTR_NODE_ID_SIZE,
    [CSAP_ATTR_NETWORK_ADDR - 1] = CSAP_ATTR_NETWORK_ADDRESS_SIZE,
    [CSAP_ATTR_NETWORK_CHANNEL - 1] = CSAP_ATTR_NETWORK_CHANNEL_SIZE,
    [CSAP_ATTR_NODE_ROLE - 1] = CSAP_ATTR_NODE_ROLE_SIZE,
    [CSAP_ATTR_APP_MAXT_TRANS_UNIT - 1] = CSAP_ATTR_APDU_SIZE_SIZE,
    [CSAP_ATTR_PDU_BUFF_SIZE - 1] = CSAP_ATTR_PDU_BUFF_SIZE_SIZE,
    [CSAP_ATTR_SCRATCHPAD_SEQ - 1] = CSAP_ATTR_SCRATCHPAD_SEQ_SIZE,
    [CSAP_ATTR_WAPS_VERSION - 1] = CSAP_ATTR_WAPS_VERSION_SIZE,
    [CSAP_ATTR_FIRMWARE_MAJOR - 1] = CSAP_ATTR_FIRMWARE_MAJOR_SIZE,
    [CSAP_ATTR_FIRMWARE_MINOR - 1] = CSAP_ATTR_FIRMWARE_MINOR_SIZE,
    [CSAP_ATTR_FIRMWARE_MAINTENANCE - 1] = CSAP_ATTR_FIRMWARE_MAINTENANCE_SIZE,
    [CSAP_ATTR_FIRMWARE_DEVELOPMENT - 1] = CSAP_ATTR_FIRMWARE_DEVELOPMENT_SIZE,
    [CSAP_ATTR_CIPHER_KEY - 1] = CSAP_ATTR_CIPHER_KEY_SIZE,
    [CSAP_ATTR_AUTHENTICATION_KEY - 1] = CSAP_ATTR_AUTHENTICATION_KEY_SIZE,
    [CSAP_ATTR_CHANNEL_LIMITS - 1] = CSAP_ATTR_CHANNEL_LIMIT_SIZE,
    [CSAP_ATTR_APPCFG_MAX_SIZE - 1] = CSAP_ATTR_APPCFG_MAX_SIZE_SIZE,
    [CSAP_ATTR_HWMAGIC - 1] = CSAP_ATTR_HWMAGIC_SIZE,
    [CSAP_ATTR_STACK_PROFILE - 1] = CSAP_ATTR_STACK_PROFILE_SIZE,
    [CSAP_ATTR_RESERVED_1 - 1] = CSAP_ATTR_RESERVED_1_SIZE,
    [CSAP_ATTR_OFFLINE_SCAN - 1] = CSAP_ATTR_OFFLINE_SCAN_SIZE,
    [CSAP_ATTR_RESERVED_3 - 1] = CSAP_ATTR_RESERVED_3_SIZE,
    [CSAP_ATTR_FEATURE_LOCK_BITS - 1] = CSAP_ATTR_FEATURE_LOCK_BITS_SIZE,
    [CSAP_ATTR_FEATURE_LOCK_KEY - 1] = CSAP_ATTR_FEATURE_LOCK_KEY_SIZE,
    [CSAP_ATTR_RESERVED_2 - 1] = CSAP_ATTR_RESERVED_2_SIZE,
    [CSAP_ATTR_RESERVED_CHANNELS - 1] = CSAP_ATTR_RESERVED_CHANNELS_SIZE,
    [CSAP_ATTR_NETWORK_KEY_PAIR - 1] = CSAP_ATTR_KEY_PAIR_SIZE,
    [CSAP_ATTR_MANAGEMENT_KEY_PAIR - 1] = CSAP_ATTR_KEY_PAIR_SIZE,
};
    ...
    switch (attr_id)
    {
        ...
        case CSAP_ATTR_RESERVED_CHANNELS:
            ...
            *attr_size_p = attr_size;
            attr_size = 0;
            break;
        case CSAP_ATTR_NETWORK_KEY_PAIR:
        case CSAP_ATTR_MANAGEMENT_KEY_PAIR:
             result = APP_RES_RESOURCE_UNAVAILABLE;
             break;
...
static app_res_e write_key_pair(attr_t attr_id,
                                const csap_attr_key_pair_t * key_pair,
                                uint8_t attr_size)
{
    if (attr_size != CSAP_ATTR_KEY_PAIR_SIZE)
    {
    ...
    if (key_pair != NULL)
    {
        app_lib_settings_key_management_configuration_t config = {
                .network_key_pair.encryption_key_p = NULL,
                .network_key_pair.authentication_key_p = NULL,
                .management_key_pair.encryption_key_p = NULL,
                .management_key_pair.authentication_key_p = NULL,
                .flags = {
                    .apply_flags = 0, /* No flags applied - no effect on system config */
                }
        };
        ...
        {
            case CSAP_ATTR_NETWORK_KEY_PAIR:
                config.network_key_pair.encryption_key_p =
                    &key_pair->key_pair.encryption;

                config.network_key_pair.authentication_key_p =
                    &key_pair->key_pair.authentication;

                config.network_key_pair_seq = key_pair->sequence_number;
                config.management_key_pair_seq = 0;
                break;
            case CSAP_ATTR_MANAGEMENT_KEY_PAIR:
                config.management_key_pair.encryption_key_p =
                    &key_pair->key_pair.encryption;
                config.management_key_pair.authentication_key_p =
                    &key_pair->key_pair.authentication;

                config.network_key_pair_seq = 0;
                config.management_key_pair_seq = key_pair->sequence_number;
                break;
            default:
                return APP_RES_INVALID_VALUE;
        }
        return lib_settings->keyManagementConfiguration(&config);
    }
                                    ...
                                    uint8_t attr_size)
{
    ...
    switch (attr_id)
    {
        ...
        case CSAP_ATTR_NETWORK_KEY_PAIR:
        case CSAP_ATTR_MANAGEMENT_KEY_PAIR:
            result = write_key_pair(attr_id, (const csap_attr_key_pair_t*)value, attr_size);
            break;
        case CSAP_ATTR_RESERVED_1:
        case CSAP_ATTR_RESERVED_2:
        case CSAP_ATTR_RESERVED_3:
...
static bool resetReq(waps_item_t * item)
{
    ...
    {
        // Do the reset
        app_res_e res = lib_settings->resetAll();
        if (res == APP_RES_INVALID_STACK_STATE)
        {
            result = CSAP_RESET_INVALID_STATE;
        }
        else if (res == APP_RES_RESOURCE_UNAVAILABLE)
        {
            result = CSAP_RESET_RESOURCE_ERROR;
        }
        else
        {
