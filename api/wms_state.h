 ...
 *
 * Library services are accessed via [lib_state](@ref app_lib_state_t)
 * handle.
 */
 ...
 * [lib_state->getNbors()](@ref app_lib_state_get_nbors_f) service.
 */
 ...
 * configured: [device role](@ref app_lib_settings_set_node_role_f), @ref
 ...
 *          set with [lib_state->setOnStackEventCb()](@ref app_lib_state_set_stack_event_cb_f)
...
typedef enum
{
    ...
    /** Connection fallback mechanism triggered */
    APP_LIB_STATE_STACK_EVENT_CON_FALLBCK = 7  
 ...
 * @note        This can only be done on [advertisers](@ref APP_LIB_SETTINGS_ROLE_ADVERTISER) and [LE subnodes](@ref APP_LIB_SETTINGS_ROLE_SUBNODE_LE) (without autorole).
