 ...
 * 
 * If flags.key_management is set to true (1), the key management feature is enabled, provided that the management_key_pair is set either
 * by providing valid management_key_pair in this structure or it is already set in the persistent memory. 
 * 
 * If flags.key_management is set to false (0), the key management feature state is not changed, but only provided keys are set. In 
 ...
 * Key revocation will also disable the key management feature until it's activated again with flags.key_management=true.
 */
typedef struct __attribute__((packed))
{
    ...
    struct
    {
        uint8_t key_management:1;  /**< flag to enable security key management */
        uint8_t revoke_keys:1;     /**< flag to let application force key revocation */
    } flags;

    uint8_t network_key_pair_seq; /**< Sequence number, 1-255 */
    uint8_t management_key_pair_seq; /** < optional sequence number for the diagnostics */
...
/**
 * @brief       Callback for providing the node UID to the stack
 *
 * @param[out]      buffer
 *                  Pointer to buffer where the UID should be copied to.
 *                  It is at least APP_LIB_SETTINGS_MAX_NODE_UID_SIZE
 *                  bytes long.
 * @param[in,out]   length
 *                  Pointer to variable that will receive the actual length of
 *                  the UID. As input, it contains the maximum length of the
 *                  buffer and as output, the length of the UID
 *                  copied to the buffer.
 * @return          true if UID was successfully copied, false otherwise
 */
typedef bool (*app_lib_settings_get_node_uid_cb_f)(
    uint8_t buffer[APP_LIB_SETTINGS_MAX_NODE_UID_SIZE], uint32_t * length);

/**
 * @brief   Register a callback for retrieving the node UID
 *
 * This function registers a callback that will be called when the stack needs
 * to access the node UID. The UID can be queried by using the Remote API service
 * provided by stack.
 *
 * @param   cb
 *          Callback function to be called when stack needs the node UID
 * @return  Result code, @ref APP_RES_OK if successful,
 *          @ref APP_RES_INVALID_NULL_POINTER if cb is NULL
 */
typedef app_res_e (*app_lib_settings_register_node_uid_cb_f)(
    app_lib_settings_get_node_uid_cb_f cb);
