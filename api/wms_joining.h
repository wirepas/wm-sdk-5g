 ...
 *
 * Library services are accessed  via [lib_joining](@ref app_lib_joining_t)
 * handle.
 *
 */
...
#define APP_LIB_JOINING_VERSION 0x201
 ...
 * of joining beacons in [lib_joining->startJoiningBeaconTx()](@ref app_lib_joining_start_joining_beacon_tx_f) service.
 */
 ...
 * of joining beacon reception in [lib_joining->startJoiningBeaconRx()](@ref app_lib_joining_start_joining_beacon_rx_f) service.
 */
 ...
 *
 * Used in [beacon reception callback](@ref app_lib_joining_beacon_rx_cb_f) set
 * with service [lib_joining->startJoiningBeaconRx()](@ref app_lib_joining_start_joining_beacon_rx_f).
 */
typedef enum
{
    /** Reception timeout elapsed or requested number of beacons received */
    APP_LIB_JOINING_RX_STATUS_DONE = 0,
    /** Beacon reception stopped by calling [lib_joining->stopJoiningBeaconRx()](@ref app_lib_joining_stop_joining_beacon_rx_f) */
 ...
 *
 * Used in service [lib_joining->startJoiningBeaconTx()](@ref app_lib_joining_start_joining_beacon_tx_f).
 */
typedef struct
{
...
struct app_lib_joining_received_beacon_s
{
    ...
    uint32_t type;
    /** Old payload pointer for backwards compatibility */
    uint8_t * old_payload;
    /** Number of payload bytes */
    size_t payload_num_bytes;
    ...
    int8_t tx_power;
    /** To align the header structure to 32 bits for backwards compatibility */
    uint8_t reserved;
    /** Payload */
    uint8_t payload[APP_LIB_JOINING_MAX_PAYLOAD_NUM_BYTES];
};
 ...
 *
 * Used in [lib_joining->startJoiningBeaconRx()](@ref app_lib_joining_start_joining_beacon_rx_f) service.
 */
typedef struct
{
 ...
 *          interval is invalid,
            \p payload_num_bytes > 0 or \p type >= 65536
 ...
 *          parameters
 * \note    Value of the argument param->addr is ignored in 5G Mesh networks.
            The address used in joining beacons is the own network address
            of the node.
 */
