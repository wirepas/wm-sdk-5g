 ...
 * support.
 *
 * Library services are accessed via [lib_memory_area](@ref app_lib_memory_area_t) handle.
 *
 */
...
typedef enum
{
    ...
    /** External Flash driver missing, or secure storage verify error */
    APP_LIB_MEM_AREA_RES_NODRIVER = 3,
    /** Invalid parameters */
    APP_LIB_MEM_AREA_RES_PARAM = 4,
    /** Area doesn't exist */
    APP_LIB_MEM_AREA_RES_INVALID_AREA = 5,
    /** Record with tag doesn't exist (secure storage library only) */
    APP_LIB_MEM_AREA_RES_INVALID_TAG = 6,
 ...
 */
#define APP_LIB_MEM_AREA_MAX_AREAS 16
 ...
 * area with service [lib_memory_area->getAreaInfo()](@ref app_lib_mem_area_getAreaInfo_f).
 */
typedef enum
{
    /** Application area */
