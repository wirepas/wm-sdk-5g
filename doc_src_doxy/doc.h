...
[Single-MCU API Operation Principle](@ref application_operation) describes
...
[SDK Environment](@ref sdk_environment) describes the SDK package contents
  and available free processor resources for application.

[How to develop applications with Single-MCU SDK](@ref how_to_develop)
...
use [stack libraries](@ref stack_api).
...
These [libraries](@ref libraries_api) are wrappers on top of the [single MCU api](@ref single_mcu_api).
...
For HAL services offered by the SDK, see [API services by SDK](@ref mcu_api).
...
partitioning that is configured through an [.ini file](@ref config_mk_ini_file).
...
For details on this, check [EFR32 resources](@ref efr32_resources) or [Nordic nRF52XXX resources](@ref nordic_resources) according to your architecture.
