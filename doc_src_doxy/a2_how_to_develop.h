...
<code>[image/](@ref image_folder)</code> folder.
...
The provided SDK contains several [example applications](@ref source_folder).
...
information can be seen in file <code>[config.mk](@ref source_config_mk)
</code>
...
It is mandatory to have unique [app_area_id](@ref app_specific_area_id) in order
to update specific application independently on other applications. Thus,
modify file <code>[config.mk](@ref source_config_mk) </code> following by using
...
Note that the [lib_state->stopStack()](@ref app_lib_state_stop_stack_f) function
...
configuration via another interface ([UART](@ref source_makefile_hal_uart), [SPI](@ref source_makefile_hal_spi), NFC, ...). This is the case with the
<code>@ref dualmcu_app/app.c "dualmcu_app"</code> application, for example.

@note Application **must** call [lib_state->startStack()](@ref app_lib_state_start_stack_f). It can be at the end of [App_Init()](@ref app_init)
function or in a [deferred context](@ref app_lib_system_set_periodic_cb_f) but it
...
the application point of view and will generate a reboot and a new call to [App_Init()](@ref app_init) after the update.

@note Some node settings must be the same across all the nodes in the network.
More general information about node configuration can be found in [[4]](@ref relmat4).

@section build_application Build application

Building is done in root of the SDK file structure. It is done by calling the
<code> [makefile](@ref makefile)</code>. There can be many options for building but
necessary ones are in <code>[application makefile](@ref source_makefile)</code>.
For example by using <code>pca10040</code> as a [target board](@ref board_folder),
...
For detailed information on build process, see [here](@ref build).

@note Many times the application must be built on custom hardware, not yet
having board definition. See documentation [here](@ref define_custom_board).

@section test_application Test application

To practically test the application, a minimum of two boards is needed. One of
them must be configured as a [sink Low Energy](@ref APP_LIB_SETTINGS_ROLE_SINK_LE) or
a [sink Low Latency](@ref APP_LIB_SETTINGS_ROLE_SINK_LL) and the
...
[makefile](@ref makefile)</code> options used):
@code
...
To flash the image to the device, see [here](@ref flashing_guideline).
...
To implement new board, check out the documentation of [board definitions](@ref board_folder) and modify existing board template (according to processor
architecture).
...
application. It is provided for convenience and backward compatibility of
the applications.
...
as declaring it in application specific [source files](@ref source_makefile_srcs)
in <code>[makefile](@ref source_makefile)</code>, for example:
...
the <code>[INCLUDES](@ref source_makefile_includes)</code> variable in the
application <code>[makefile](@ref source_makefile)</code>.
...
This application area can be accessed through the [storage library](@ref app_lib_storage_t)
handle.
  ...
  - Cons
     -# Very limited in size. Maximum size can be asked with [lib_stoarge->getPersistentMaxSize](@ref app_lib_storage_get_persistent_max_size_f)()
...
More information about area ids and its usage can be found in [[2]](@ref relmat2)

The steps to realize are:
   -# Defining a new partitioning in the [.ini file](@ref config_mk_ini_file) by adding a new area
   with flag user (i.e. flags = 0x00000014)
   -# Access it through the [memory area library](@ref app_lib_memory_area_t) handle from your application.
...
area in @ref storage_memarea.c "storage_memarea.c" and example of modified <code>.ini</code> file in the scratchpad_ini folder
