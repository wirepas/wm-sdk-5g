...
Single-MCU API. The application is designed to have a [single-entry point](@ref app_init). This entry point is called by the Wirepas Mesh stack at
boot. The application must do its initialization during the application entry
point call.

Also, during the application [entry point call](@ref app_init), the Wirepas Mesh
...
these areas are platform dependent and described in [SDK Environment](@ref sdk_environment). The C stack is shared between the application and the Wirepas Mesh
...
Wirepas Mesh stack calls the application [entry point](@ref app_init) at this
...
-# via interrupt. There are two types
   of interrupts: [fast](@ref fast_interrupt) and [deferred interrupts](@ref deferred_interrupt).
...
Check out service for [lib_system->setPeriodicCb()](@ref app_lib_system_set_periodic_cb_f) and @ref app_scheduler.h "app scheduler library"
...
recommended maximum periods, check [guidance on maximum execution time limits](@ref execution_time_limits).
...
There are plenty of various
   ...
   context. Most common callback used and example of such callback is @ref app_lib_data_t.setDataReceivedCb "unicast data reception callback".
...
<code>[lib_system->enableAppIrq](@ref app_lib_system_enable_app_irq_with_handler_f)</code> service and <code>[lib_system->disableAppIrq](@ref app_lib_system_disable_app_irq_f) </code> service.
...
time is set to 100us, see also [guidance on maximum execution time limits](@ref execution_time_limits).
...
scheduling. See also [guidance on maximum execution time limits](@ref execution_time_limits).
...
<code>[lib_system->enterCriticalSection()](@ref app_lib_system_enter_critical_section_f)</code> and <code> [lib_system->exitCriticalSection()](@ref app_lib_system_exit_critical_section_f)</code>.
...
@ref periodic_application periodic application callback (using [lib_system->setPeriodicCb()](@ref app_lib_system_set_periodic_cb_f) service or @ref app_scheduler.h "app scheduler" library) and call services from there. Services
