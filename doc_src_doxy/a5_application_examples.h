...
- [Example applications](@ref example_applications) which can be used as a
...
- [Demos and production apps](@ref production_apps). They are applications that
...
</code> and used hardware is Nordic nRF52 (i.e. @ref app_lib_system_hardware_magic_e is @ref APP_LIB_SYSTEM_HARDWARE_MAGIC_NRF52832_SP_2), the resulted area id stored in the
devices is:
@code
...
This configuration defines the [boards](@ref board_folder) which application
...
- @subpage source_makefile_ldlibs
...
defined in specific [board](@ref board_folder).
...
in specific [board](@ref board_folder).
...
@note You need to define <code>-DUSE_PERSISTENT_MEMORY</code> by using [CLAGS](@ref source_makefile_cflags) option.
...
These flags are placed in the beginning of the linker command line. Prefix
<code>-Wl,</code> is required for each separate parameter added here, or the
parameters can be in a comma-separated list after a single <code>-Wl,</code>.

Example:
@code
# Print the names of the input files and archive members as ld processes them
LDFLAGS += -Wl,--trace,--trace
@endcode

@subsection source_makefile_ldlibs LDLIBS

Adding compiler-provided libraries can be done by extending this list.
The libraries are added to the end of the linker command line, so they are
usable from precompiled libraries (see \ref source_makefile_libs) as well.
...
Example:
@code
LIBS += $(APP_SRCS_PATH)my_lib/my_lib.a
@endcode
...
- @subpage name_to_uint32_py

@subsection genscratchpad_py genscratchpad.py

Wirepas Mesh wireless updates, or Over-The-Air-Programming ("OTAP"), use a file
format called a "scratchpad". A scratchpad file contains one or more compressed
and encrypted files, each of which have a specific target area ID that the file
is meant for.

A tool called <code>genscratchpad.py</code> is used to generate scratchpad
files. It reads Intel HEX files as inputs, compresses and encrypts them
according to a specified <code>.ini</code> file, and writes out the resulting
scratchpad file. Scratchpad files have a suffix of <code>.otap</code>.

<code>genscratchpad.py</code> is normally called automatically as part of an
application build ([Makefile](@ref makefile)), but it can also be called
manually, or from user-provided scripts. Various special scratchpad files can be
created this way, for advanced OTAP use.

@subsection config_mk_ini_file Bootloader and scratchpad .ini files

Scratchpad files are compressed and encrypted for use with a specific Wirepas
Mesh network. When a device reeives new scratchpad contents, the bootloader
authenticates the scratchpad and decrypts it. Aauthentication and encryption
keys used to generate a scratchpad must match the ones stored in the bootloader,
or the scratchpad will not be processed further.

The list of keys stored in bootloader can be configured in the
<code>custom_bootloader_keys.ini</code> file. The bootloader can contain
several keys and will decrypt a received scratchpad, if it can be authenticated
with any of the listed keys.

The bootloader has a number of preconfigured memory areas. When it processes a
scratchpad, the area IDs of files within are compared to the areas known by the
bootloader, and processed when matching files are found.

By default, each application uses the same <code>\<mcu\>_app.ini</code> file
in the <code>mcu/</code> folder, but the <code>.ini</code> file can be copied to
the application folder and modified as needed. The new <code>.ini</code> file
can then be specified in the application <code>config.mk</code> file, as
follows:

@code
    # Define a specific application ini file
    INI_FILE_APP = $(APP_SRCS_PATH)/<mcu>_app.ini
@endcode

The scratchpad generated from an application build uses a key called
<code>default</code> from the <code>custom_bootloader_keys.ini</code> file.
Another key can be used instead, by adding the <code>--keyname</code> option to
the invocation of <code>genscratchpad.py</code> in user scripts.

@subsection name_to_uint32_py name_to_uint32.py

The @ref wms_secure_storage.h "secure storage library" uses 32-bit tags instead
of filenames for efficiency. To achieve this, it employs a character encoding
scheme called [SQUOZE](https://en.wikipedia.org/wiki/DEC_Radix-50),
which can store up to six characters in a 32-bit integer. The available
character set consists of 40 symbols:

@code{.sh}
    ABCDEFGHIJKLMNOPQRSTUVWXYZ$.%0123456789
@endcode

In addition, a space character is supported, but it cannot appear as the first
character in the name.

Any listed names are converted to integers. If a single name is given, the tool
converts and prints the result without extra text, making this mode suitable
for use in scripts. When multiple names are given, both the names and their
converted values are printed, separated by newlines.

The command-line flag @c -u or @c --unsquoze converts integers back to names.

The command-line flag @c -d or @c --decimal displays all values in decimal
instead of hexadecimal. Values on the command line may be provided in either
format, with @c 0x indicating a hexadecimal value.

Convert a single name to uint32:
@code{.sh}
    $ tools/name_to_uint32.py myvar
    0x0214c3ba
@endcode

Convert multiple names:
@code{.sh}
    $ tools/name_to_uint32.py this is a test
    THIS: 0x0013bb7b
    IS: 0x0000017b
    A: 0x00000001
    TEST: 0x0013aa4c
...
@code{.sh}
    tools/name_to_uint32.py -u -d 0x0013bb7b 0x0000017b 0x00000001 0x0013aa4c
    1293179: THIS
    379: IS
    1: A
    1288780: TEST
...
/** @example battery_voltage_read_app/app.c
 */
...
/** @example app_persistent/app.c
 */

/** @example diradv/app.c
 */

/** @example local_provisioning/app.c
 */
