...
bool info_keys(const memory_area_services_t * mem_area_services)
{
...
#else //EFR32                                   // DEVICE
#if (_SILICON_LABS_32B_SERIES == 1)             // SERIES
    uint8_t * keys = (uint8_t *)(0x7D00);

#elif (_SILICON_LABS_32B_SERIES == 2)           // SERIES

#if (_SILICON_LABS_32B_SERIES_2_CONFIG == 1 || _SILICON_LABS_32B_SERIES_2_CONFIG == 2 )    // SERIES2_CONFIG
#ifdef BOOT_TEST_OMAC1_AES128CTR
    uint8_t * keys = (uint8_t *)(0x3C80);
#else
    uint8_t * keys = (uint8_t *)(0x08007D00);
#endif


#elif( _SILICON_LABS_32B_SERIES_2_CONFIG == 3 || _SILICON_LABS_32B_SERIES_2_CONFIG == 4 )

#ifdef BOOT_TEST_OMAC1_AES128CTR
    uint8_t * keys = (uint8_t *)(0x08007C80);
#else
    uint8_t * keys = (uint8_t *)(0x08007D00);
#endif

#endif                                          // SERIES2_CONFIG
#endif                                          // SERIES
#endif                                          // DEVICE
    ...
    for(uint32_t i=0; i < NUM_KEY_PAIRS; i++)
    {
        ...
        for(uint32_t j=0; j < ENCRYPTION_KEY_LENGHT; j++)
        {
            Print_printf("[%02X]",*(keys + (2*i+1)*ENCRYPTION_KEY_LENGHT+j));
        }
        Print_printf("\n");
