...
#include "external_flash_gd25le255e.h"
#include "nrf_gpio.h"
#include "nrf_spim.h"
...
static const flash_info_t m_flash_info = {
    ...
    .write_alignment = 1,
    .byte_write_time = 60,                // Typical 30 us, Max 60 us. (first byte)
    .page_write_time = 2400,              // Typical 250 us, Max 2.4 ms.
    .sector_erase_time = 1200000,         // Typical 100 ms, Max 1.2 s.
...
static bool ext_flash_write_enable()
{
    ...
    cmd[0] = GD25LE255E_CMD_WRITE_ENABLE;
    ...
    cmd[0] = GD25LE255E_CMD_READ_STATUS1;
    ext_flash_select(true);
    ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
    ext_flash_select(false);
    return (reply[1] & GD25LE255E_STATUS_WEL) == GD25LE255E_STATUS_WEL;
}
...
extFlash_res_e externalFlash_init(void)
{
    ...
    /* Both GD25LE255E (DK) and MX25R6435F (Thingy:X) support only SPI modes 0 and 3. */
    ...
    {
        cmd[0] = GD25LE255E_CMD_READ_IDENTIFICATION;

        ext_flash_select(true);
        ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
        ext_flash_select(false);

        if ((reply[1] == GD25LE255E_ID_1) &&
            (reply[2] == GD25LE255E_ID_2) &&
            (reply[3] == GD25LE255E_ID_3))
        {
...
extFlash_res_e externalFlash_startRead(void * to, const void * from, size_t amount)
{
    ...
    cmd[0] = GD25LE255E_CMD_READ_ARRAY;
    ...
    while (amount > 0) {
        uint32_t bytesToRead = MIN(amount, GD25LE255E_MAX_TRANSFER_SIZE);
...
extFlash_res_e externalFlash_startWrite(void * to, const void * from, size_t amount)
{
    ...
    cmd[0] = GD25LE255E_CMD_PROGRAM_PAGE;
    set_address(&cmd[1], address);
    ...
    while (amount > 0)
    {
        uint32_t bytesToWrite = MIN(amount, GD25LE255E_MAX_TRANSFER_SIZE);
...
bool externalFlash_isBusy(void)
{
    ...
    ext_flash_spi_xfer(cmd, sizeof(cmd), reply, sizeof(reply), false);
    ext_flash_select(false);
    return (reply[1] & GD25LE255E_STATUS_WIP) == GD25LE255E_STATUS_WIP;
}
...
extFlash_res_e externalFlash_startErase(size_t * sector_base, size_t * number_of_sector)
{
    ...
    cmd[0] = GD25LE255E_CMD_BLOCK_ERASE_32K;
    set_address(&cmd[1], address);
