...
void App_init(const app_global_functions_t * functions)
{
    ...
    else
    {
        ...
        uid_len = Storage_getUID(&conf.settings.prov_uid.uid[0]);
        Storage_getAuthKey(&conf.settings.prov_keys.auth_key[0]);
        Storage_getEncKey(&conf.settings.prov_keys.enc_key[0]);
        method = Storage_getMethod();

        if (uid_len > 0)
        {
            conf.settings.prov_uid_len = uid_len;
            conf.settings.method = method;
            Provisioning_init(&conf);
        }
        else
        {
            LOG(LVL_ERROR, "Can't retrieve node UID from storage.");
        }
    }
