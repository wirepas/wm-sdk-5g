...
*     key.
*
*     Scratchpad authentication relies on either the symmetric OMAC1/CMAC algorithm
*     with a 128-bit key or the public key-based ECDSA P-256 algorithm.
*     Bootloaders on some architectures may support only one of these methods
*
...
def generate_random_key():
    ...
    """
    # Generate 16 random bytes (=128bits) and return as bytearray
    return bytearray(get_random_bytes(16))
...
def create_bootloader_key_pair():
    ...
    while True:
        choice = input("How do you want to generate your keys? [M]anually, [R]andomly?\n")
        if choice in ("R", "M"):
            break

    if choice == "R":

        # Generate random symmetric authentication and encryption keys
        auth = generate_random_key()
