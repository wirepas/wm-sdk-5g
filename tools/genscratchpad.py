...
class InFile(object):
    ...
    def __init__(
        ...
        if data:
            ...
            if filename_ext == ".hex":
                ...
                if memory.num_ranges == 0:
                    ...
                    memory.max_address - memory.min_address
                    > self.MAX_NUM_BYTES_PER_FILE
                ):
        ...
        self.header = self.make_file_header(self.area_id, len(self.data), *self.version)
    ...
    def compress(self):
        ...
        if self.compressible:
            ...
            num_blocks = (comp_data_len + self.BLOCK_LENGTH - 1) // self.BLOCK_LENGTH
            ...
            num_blocks = (data_pld_len + self.BLOCK_LENGTH - 1) // self.BLOCK_LENGTH
        ...
        self.header = self.make_file_header(
            self.area_id, data_length, *self.version, pad
        )
    ...
    def parse_file_spec(file_spec):
        ...
        try:
            ...
            if fields.__len__() == 3:
                ...
                raise ValueError(
                    "invalid input file specification: " "'%s'" % file_spec
                )
            ...
            if version.endswith(".conf"):
                ...
                for section in cp.sections():
                    for option in cp.options(section):
                        ...
                        if option == "modem_version":
                            modem_version = cp.get(section, option)
            ...
            # In case of modem firmware section, override version with modem version
            if area_id == 0x30000012 or area_id == 0x30000112:
                version = modem_version

            if area_id < 0 or area_id > 4294967295:
                raise ValueError
        except ValueError:
            raise ValueError("invalid input file specification: " "'%s'" % file_spec)
    ...
    def make_file_header(
        areaid, length, ver_major, ver_minor, ver_maint, ver_devel, pad=0x00000000
    ):
        """
        From bootloader.h:
        ...
        return struct.pack(
            "<2L4BL", areaid, length, ver_major, ver_minor, ver_maint, ver_devel, pad
        )
...
class SignatureTLVFile(object):
    ...
    def add_auth_algo(self, algo):
        ...
        auth_algo_rec = (
            struct.pack("<BB", self.REC_TYPE_AUTH_ALGO, len(auth_algo_id))
            + auth_algo_id
        )
    ...
    def add_encrypt_algo(self, algo):
        ...
        encrypt_algo_rec = (
            struct.pack("<BB", self.REC_TYPE_ENCRYPT_ALGO, len(encrypt_algo_id))
            + encrypt_algo_id
        )
    ...
    def add_signature(self, signature=None):
        ...
        signature_rec = (
            struct.pack("<BB", self.REC_TYPE_SIGNATURE, len(signature)) + signature
        )
...
class Scratchpad(object):
    ...
    # Size of scratchpad header, in bytes.
    SCRATCHPAD_HEADER_SIZE = 16

    # Size of secure header and Initial Counter Block (ICB), in bytes.
    SECURE_HEADER_SIZE = 16
    ...
    def __init__(self, key, platform, secure_header=None):
        self.key = key
        self.platform = platform
        self.cmac = bytearray(self.CMAC_SIZE)
        self.data = []  # List of scratchpad file header and data blocks

        if secure_header is None:
            # Generate random bytes for secure header and Initial Counter Block
            self.secure_header = get_random_bytes(self.SECURE_HEADER_SIZE)
        else:
            if len(secure_header) != self.SECURE_HEADER_SIZE:
                raise ValueError("invalid secure header size")
        ...
        if (
            secure_header is None
            and key.key_type == KeyDesc.KEY_TYPE_SHA256_ECDSA_P256_AES128CTR
        ):
    ...
    def create_cipher(self):
        ...
        if self.platform.aes_little_endian == False:
            initctr = (
                htonl(icb3) | htonl(icb2) << 32 | htonl(icb1) << 64 | htonl(icb0) << 96
            )
        else:
        ...
        ctr = Counter.new(
            128,
            little_endian=self.platform.aes_little_endian,
    ...
    def make_header(self):
        ...
        length = self.CMAC_SIZE + len(self.secure_header) + sum(map(len, self.data))
...
def ini_file(string_file):
    if not string_file.endswith(".ini"):
        raise argparse.ArgumentTypeError(
            "invalid ini file extension '%s'" % string_file
        )
    return string_file
...
def create_argument_parser(pgmname):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.fill(
            "A tool to generate compressed and " "encrypted scratchpad contents",
            help_width,
        ),
        epilog="values:\n"
