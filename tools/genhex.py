...
from bootloader_config import BootloaderConfig, KeyDesc, SIZE_ECDSA_PUBLIC_KEY_IN_BYTES
...
class Flashable(object):
    ...
    def make_bl_info_header(self, in_file):
        ...
        written_size = len(in_file.raw_data)

        # If needed, written_size in memory area is rounded up to erase block boundary.
        if self.config.flash.eraseblock > 0:
            written_size = \
                (int(written_size / self.config.flash.eraseblock) + 1) * \
    ...
    def gen_bootloader(self, bootloader):
        ...
        for key in self.config.keys.values():
            if key_type == KeyDesc.KEY_TYPE_OMAC1_AES128CTR:
                ...
                if (
                    len(auth_key) != SIZE_ECDSA_PUBLIC_KEY_IN_BYTES
                    or auth_key[0] != 0x04
                ):
